...
import {SomeProxy, ISomeProxy} from "contracts/SomeProxy.sol";
...
contract SomeProxyTest is Test {
    ...
    function test_constructor_initialization() public {
        ...
        (success, data) = address(proxy).call(
            abi.encodeWithSelector(MockImplementationV1.getName.selector)
        );
    ...
    function test_constructor_without_initialization_data() public {
        ...
        vm.prank(admin);
        SomeProxy newProxy = new SomeProxy(
            address(implementationV1),
            admin,
            ""
        );
    ...
    function test_constructor_admin_change() public {
        ...
        address erc1967Admin = ERC1967Utils.getAdmin(address(proxy));
        assertEq(erc1967Admin, admin);
    }
    ...
    function test_proxy_state_modification() public {
        ...
        (bool success,) = address(proxy).call(
            abi.encodeWithSelector(MockImplementationV1.setValue.selector, newValue)
        );
    ...
    function test_proxy_revert_propagation() public {
        ...
        vm.prank(user1);
        (bool success,) = address(proxy).call(
            abi.encodeWithSelector(MockImplementationV1.revertFunction.selector)
        );
    ...
    function test_admin_can_upgrade() public {
        ...
        bytes memory upgradeData = abi.encodeWithSelector(
            MockImplementationV2.initialize.selector,
            300,
            "Upgraded Name"
        );
        ...
        (bool success, bytes memory data) = address(proxy).call(
            abi.encodeWithSelector(MockImplementationV2.isUpgraded.selector)
        );
        ...
        (success, data) = address(proxy).call(
            abi.encodeWithSelector(MockImplementationV2.newFunction.selector)
        );
    ...
    function test_admin_cannot_call_non_upgrade_functions() public {
        ...
        ISomeProxy(address(proxy)).implementation();
    }
    ...
    function test_upgrade_with_initialization() public {
        ...
        (success, data) = address(proxy).call(
            abi.encodeWithSelector(MockImplementationV2.getName.selector)
        );
        require(success, "Failed to call getName");
        string memory name = abi.decode(data, (string));
    ...
    function test_upgrade_with_payable_initialization() public {
        // Test upgrade with ETH sent (should work with empty data)
        uint256 ethAmount = 0.1 ether;

        vm.deal(admin, ethAmount);

        vm.prank(admin);
        ISomeProxy(address(proxy)).upgradeToAndCall{value: ethAmount}(
            address(implementationV2),
            ""
        );
    ...
    function test_upgrade_fails_with_data_and_value() public {
        ...
        vm.prank(admin);
        vm.expectRevert();
        ISomeProxy(address(proxy)).upgradeToAndCall{value: 0.1 ether}(
            address(implementationV2),
            upgradeData
        );
    }
    ...
    function test_erc1967_compliance() public {
        ...
        assertEq(proxy.implementation(), ERC1967Utils.getImplementation(address(proxy)));
        assertEq(proxy.proxyAdmin(), ERC1967Utils.getAdmin(address(proxy)));
    }
    ...
    function test_full_upgrade_cycle() public {
        ...
        (success,) = address(proxy).call(
            abi.encodeWithSelector(MockImplementationV1.setValue.selector, 250)
        );
        assertTrue(success);
