...
/// @notice Interface for the UIHelper contract that facilitates token creation, buying, and selling with OpenOcean
/// integration
interface IUIHelper is IOpenOceanCaller {
  ...
  function createAndBuy(
    ...
    address _expected,
    uint256 _amount
  ) external payable returns (address token, uint256 received, uint256 swapped, uint256 tokenId);
